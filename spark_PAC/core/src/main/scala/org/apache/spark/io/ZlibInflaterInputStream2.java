 ...
 */
public
class ZlibInflaterInputStream2 extends FilterInputStream {
     ...
     */
    public ZlibInflaterInputStream2(InputStream in, Inflater inf, int size) {
        super(in);
        if (in == null || inf == null) {
     ...
     * @param inf the decompressor ("inflater")
     */
    public ZlibInflaterInputStream2(InputStream in, Inflater inf) {
        this(in, inf, 512);
    }
     ...
     * @param in the input stream
     */
    public ZlibInflaterInputStream2(InputStream in) {
        this(in, new Inflater());
        usesDefaultInflater = true;
    }

    public ZlibInflaterInputStream2(InputStream in, int size, long stream_id) {
        this(in, new Inflater(), size);
        usesDefaultInflater = true;
    ...
    public int pre_read() throws IOException {
        ...
        if (remaining_elements == 0){
            try {
                    ...
                    if (n == 0) {
                        if (inf.finished() || inf.needsDictionary()) {
                            ...
                            if (this.compression_tracer_fos != null && this.compression_performance_trace.equals("t")) {
                                String content = "pr," + (end_time - start_time) + "," + (start_time - this.stream_id) + "," + 0 +"\n";
                                byte[] input = content.getBytes(StandardCharsets.UTF_8);
                                this.compression_tracer_outputStream.write(input,0,input.length);
                            }
                            return -1;
                ...
                long end_time = System.nanoTime();
                if (this.compression_tracer_fos != null && this.compression_performance_trace.equals("t")) {
                    String content = "pr," + (end_time - start_time) + "," + (start_time - this.stream_id) + "," + n +"\n";
                    byte[] input = content.getBytes(StandardCharsets.UTF_8);
                    this.compression_tracer_outputStream.write(input,0,input.length);
                }
    ...
    public int copy(byte[] b, int off, int len) throws IOException {
        ...
        long end_time = System.nanoTime();
        if (this.compression_tracer_fos != null && this.compression_performance_trace.equals("t")) {
            String content = "cp," + (end_time - start_time) + "," + (start_time - this.stream_id) + ","+num + "\n";
            byte[] input = content.getBytes(StandardCharsets.UTF_8);
            this.compression_tracer_outputStream.write(input,0,input.length);
        }
