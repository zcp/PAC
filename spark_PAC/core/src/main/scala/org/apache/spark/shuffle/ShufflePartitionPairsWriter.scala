...
import java.io.{BufferedWriter, Closeable, FileWriter, IOException, OutputStream}
...
private[spark] class ShufflePartitionPairsWriter(
  ...
  private var buffer_writer: BufferedWriter = null
  private var limit = 0
  private val shuffle_compression_raw_sample: String = SparkEnv.get.conf.get("spark.shuffle.compression.raw.sample")
  ...
  override def write(key: Any, value: Any): Unit = {
    ...
    if(shuffle_compression_raw_sample.equals("t") && limit < 100) {
      buffer_writer.write("-1" +"," + key.toString().length() + "," + key.getClass() + "," + value.toString().length() + "," + value.getClass() +  "\n" )
      // System.out.print("zcp, key,value," + key + "," + value)
      // System.out.print("\n")
      limit += 1
    }
    recordWritten()
  ...
  private def open(): Unit = {
    try {
      ...
      // zcp
      if (this.shuffle_compression_raw_sample.equals("t")) try {
        val write_content_file = "/home/compression_tracer/sortShuffleWriter_write_raw_content-" + System.nanoTime() + ".csv"
        buffer_writer = new BufferedWriter(new FileWriter(write_content_file))
        val content = "shuffleID, the length of key,the class of key, the length of value,the class of value"
  ...
  override def close(): Unit = {
    // zcp
    if (shuffle_compression_raw_sample.equals("t")) {
       buffer_writer.close()
    }
    // zcp
    if (!isClosed) {
