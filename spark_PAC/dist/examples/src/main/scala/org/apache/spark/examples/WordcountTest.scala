...
object WordcountTest {
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder
      .appName("Wordcount-" + compression_algorithm)
    ...
    System.out.println(compression_algorithm, compression_buffersize, compression_flag,
    ...
    val counts = spark.read.textFile(input).rdd.flatMap(line => line.split(" "))
                      .map(word => (word, 1))
                      .reduceByKey(_ + _)
